...
namespace gts3
{
    ...
    public partial class Form23 : Form
    {
        ...
        public float L, Hpl, H1, m1, m2, b, z, delta_v, delta_n;
        ...
        public void pictureBox1_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (j_x = 0; j_x <=  L+50/M_x; j_x += stepM / 2)
            {
                ...
                Font ft = new Font("",10);
                msg = "Схема к фильтрационному расчету плотины с ядром по методу Е.А.Замарина";
            ...
            float xdd1=L - Hpl * m2- b;
            PointF yadrp1 = new PointF(O_x_pix + xdd1*M_x, O_y_pix);
            PointF yadrp2 = new PointF(O_x_pix + xdd1*M_x+(delta_n/2-delta_v/2)*M_x, O_y_pix-0.95f*Hpl*M_y);
            PointF yadrp3 = new PointF(O_x_pix + xdd1*M_x+(delta_n/2+delta_v/2)*M_x, O_y_pix-0.95f*Hpl*M_y);
            PointF yadrp4 = new PointF(O_x_pix + xdd1*M_x+delta_n*M_x, O_y_pix);
            PointF [] myYadrPoints = {yadrp1, yadrp2, yadrp3, yadrp4};
            g.DrawLines(yadrPen, myYadrPoints);//рисуем ядро


            Pen graphPen = new Pen(Color.Black, 3);
            float xdd2 = xdd1 + delta_n;
            float mz =((delta_n-delta_v)/2)/(0.95f*Hpl);
            float xd1 = x1 + y1 * mz;
            float xd2 = xdd2-y2*mz;            
            float xd6 = L-y6*m2;
           
            PointF point1 = new PointF(O_x_pix + xd1* M_x, O_y_pix - y1 * M_y);
            PointF point2 = new PointF(O_x_pix + xd2 * M_x, O_y_pix - y2 * M_y);
            PointF point3 = new PointF(O_x_pix+(xd2+(xd6-xd2)/4)*M_x, O_y_pix - y3 * M_y);
            PointF point4 = new PointF(O_x_pix+(xd2+(xd6-xd2)*2/4)*M_x, O_y_pix - y4 * M_y);
            PointF point5 = new PointF(O_x_pix+(xd2+(xd6-xd2)*3/4)*M_x, O_y_pix - y5 * M_y);           
            PointF point6 = new PointF(O_x_pix+xd6*M_x, O_y_pix - y6 * M_y);//точки графика 
            PointF[] myLinesPoints = {point2, point3, point4, point5, point6};
            ...
            g.DrawLine(water2Pen, w2p1, point1);//кусочек от линии при H1 до кривой
